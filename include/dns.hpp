...
std::ostream &operator<<(std::ostream &os, const DNSQuery &query) {
  ...
  os << "|       Query Type | " << std::left << std::setw(17)
     << QTypeToString((QType)query.type) << " |" << std::endl;
  os << "|      Query Class | " << std::left << std::setw(17)
     << QClassToString((QClass)query.qclass) << " |" << std::endl;
  os << "+------------------+-------------------+" << std::endl;
  return os;
}
...
std::ostream &operator<<(std::ostream &os, const DNS &packet) {
  os << "|                 DNS                  |" << std::endl;
  os << "+------------------+-------------------+" << std::endl;
  os << "|   Transaction ID | 0x" << std::left << std::setw(15) << std::hex
     << packet.transactionId << std::dec << " |" << std::endl;
  os << "|            Flags | 0x" << std::left << std::setw(15) << std::hex
     << packet.flags << std::dec << " |" << std::endl;
  os << "|        Questions | " << std::left << std::setw(17)
     << packet.queriesCount << " |" << std::endl;
  os << "|       Answer PRs | " << std::left << std::setw(17)
     << packet.answersCount << " |" << std::endl;
  os << "|    Authority PRs | " << std::left << std::setw(17)
     << packet.authoritiesCount << " |" << std::endl;
  os << "|   Additional PRs | " << std::left << std::setw(17)
     << packet.additionalsCount << " |" << std::endl;
  os << "+------------------+-------------------+" << std::endl;
  os << "|               Queries                |" << std::endl;
